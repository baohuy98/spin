...
export default function ViewerPage() {
    ...
    // Socket.io for room management
    const { socket, isConnected, roomData, joinRoom, onSpinResult } = useSocket()

    // WebRTC for receiving screen share
    const { remoteStream, error } = useWebRTC({
        socket,
        roomId: roomData?.roomId || null,
        isHost: false
    })
    ...
    useEffect(() => {
        if (isConnected && roomId) {
            // Generate a random viewer ID (or you can use a member selection flow)
